<?php
/**
 * LectorThema - Meta Boxes y Campos Personalizados
 *
 ...
 */
function lectorthema_add_meta_boxes() {
    // Meta box para Obras (Manga)
    add_meta_box(
        'lectorthema_details_box',
        __('Información Detallada de la Obra', 'lectorthema'),
        'lectorthema_render_manga_meta_box',
        'manga',
        'normal',
        'high'
    );
    ...
    add_meta_box(
        'lectorthema_chapter_box',
        __('Detalles del Capítulo y Lector', 'lectorthema'),
        'lectorthema_render_chapter_meta_box',
        'chapter',
        'normal',
        'high'
    );
}
add_action('add_meta_boxes', 'lectorthema_add_meta_boxes');

/**
 * 2. Renderizado del Meta Box de Manga
 */
function lectorthema_render_manga_meta_box($post) {
    wp_nonce_field('lectorthema_meta_nonce_action', 'lectorthema_meta_nonce');
    ...
    <div class="manga-row-flex">
        <div class="manga-field-group" style="flex: 1; min-width: 250px;">
            <label for="_manga_alt_titles"><?php _e('Títulos Alternativos / Originales', 'lectorthema'); ?></label>
        ...
        <div class="manga-field-group" style="flex: 1; min-width: 200px;">
            <label for="_manga_release_year"><?php _e('Fecha / Año de Lanzamiento', 'lectorthema'); ?></label>
    ...
    <div class="manga-row-flex">
        <div class="manga-field-group" style="flex: 1; min-width: 200px;">
            <label for="_manga_author"><?php _e('Autor(es)', 'lectorthema'); ?></label>
            <input type="text" id="_manga_author" name="_manga_author" value="<?php echo esc_attr($author); ?>" placeholder="Ej: Redice Studio">
        </div>

        <div class="manga-field-group" style="flex: 1; min-width: 200px;">
            <label for="_manga_artist"><?php _e('Artista(s)', 'lectorthema'); ?></label>
        ...
        <div class="manga-field-group" style="flex: 1; min-width: 150px;">
            <label for="_manga_rating"><?php _e('Puntuación Inicial (1 - 10)', 'lectorthema'); ?></label>
    ...
    <div class="manga-field-group">
        <label for="_manga_banner_url"><?php _e('URL de Imagen Banner (Fondo Panorámico)', 'lectorthema'); ?></label>
        ...
        <p class="manga-field-desc"><?php _e('Imagen ancha para el fondo de la ficha o slider.', 'lectorthema'); ?></p>
    </div>

    <div class="manga-row-flex">
        <div class="manga-field-group" style="flex: 1;">
            <label for="_manga_badge_icon"><?php _e('Emblema / Sello de Portada (Opcional)', 'lectorthema'); ?></label>
            ...
            <p class="manga-field-desc"><?php _e('Emblema circular en la esquina superior izquierda de la tarjeta.', 'lectorthema'); ?></p>
        </div>
        ...
        <div class="manga-field-group" style="flex: 1; display: flex; align-items: center; margin-top: 20px;">
            <label style="cursor: pointer;">
                ...
                <strong><?php _e('⭐ Destacar en el Slider Principal Superior', 'lectorthema'); ?></strong>
            </label>
        </div>
    </div>
    <?php
 ...
 */
function lectorthema_render_chapter_meta_box($post) {
    wp_nonce_field('lectorthema_chapter_nonce_action', 'lectorthema_chapter_nonce');
    ...
    <div class="manga-row-flex">
        <div class="manga-field-group" style="flex: 2; min-width: 300px;">
            <label for="_chapter_manga_id"><?php _e('Selecciona la Obra / Manga Perteneciente *', 'lectorthema'); ?></label>
            <select id="_chapter_manga_id" name="_chapter_manga_id" required style="max-width: 100%;">
                <option value=""><?php _e('-- Seleccionar Manga --', 'lectorthema'); ?></option>
        ...
        <div class="manga-field-group" style="flex: 1; min-width: 150px;">
            <label for="_chapter_number"><?php _e('Número de Capítulo *', 'lectorthema'); ?></label>
        ...
        <div class="manga-field-group" style="flex: 1; min-width: 150px; display: flex; align-items: center; margin-top: 25px;">
            <label style="cursor: pointer;">
                ...
                <strong style="color: #e11d48;">🔥 <?php _e('Marcar como Capítulo HOT / Nuevo', 'lectorthema'); ?></strong>
            </label>
        </div>
    </div>

    <div class="manga-field-group">
        <label for="_chapter_subtitle"><?php _e('Título / Subtítulo del Capítulo (Opcional)', 'lectorthema'); ?></label>
    ...
    <div class="manga-field-group">
        <label for="_chapter_images"><?php _e('Imágenes del Capítulo (Una URL por línea para el Lector Webtoon / Manga)', 'lectorthema'); ?></label>
        ...
        <p class="manga-field-desc"><?php _e('Pega las URLs de las páginas del capítulo en orden descendente. El lector las cargará de forma fluida.', 'lectorthema'); ?></p>
    </div>
    <?php
}
 ...
 */
function lectorthema_save_meta_data($post_id) {
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
    if (!current_user_can('edit_post', $post_id)) return;

    // Guardado de Manga
    if (isset($_POST['lectorthema_meta_nonce']) && wp_verify_nonce($_POST['lectorthema_meta_nonce'], 'lectorthema_meta_nonce_action')) {
        $fields = [
    ...
    if (isset($_POST['lectorthema_chapter_nonce']) && wp_verify_nonce($_POST['lectorthema_chapter_nonce'], 'lectorthema_chapter_nonce_action')) {
