<?php
/**
 * LectorThema - Template Part: Top de la Comunidad (Grilla 3x3 en Móvil y Capítulos Lado a Lado)
 *
 * Características:
...
$top_weekly  = lectorthema_get_top_weekly(9);
$top_daily   = lectorthema_get_top_daily(9);
$top_monthly = lectorthema_get_top_monthly(9);
$top_alltime = lectorthema_get_top_alltime(9);
    ...
    <div class="top-rankings-grid <?php echo $is_active ? '' : 'is-hidden'; ?>" id="<?php echo esc_attr($list_id); ?>" style="<?php echo $is_active ? 'display: grid;' : 'display: none;'; ?>">
        ...
        <?php else: ?>
            <div style="grid-column: 1 / -1; text-align: center; padding: 35px 20px; color: var(--text-muted); font-size: 14px; background: var(--surface-secondary); border-radius: var(--radius-xs); border: 1px solid var(--border);">
                ...
                <p><?php _e('Actualizando estadísticas de la comunidad...', 'lectorthema'); ?></p>
            </div>
        <?php endif; ?>
    </div>
    <?php
...
<section class="top-community-section" style="margin-bottom: 35px;">
    <div class="section-header-block" style="margin-bottom: 16px;">
        <h2 class="section-title" style="font-size: 19px;">
            <i class="fa-solid fa-ranking-star"></i> <?php _e('Top de la Comunidad', 'lectorthema'); ?>
        </h2>

        <!-- Navegación por Pestañas -->
        <div class="top-tabs-nav" style="margin-bottom: 0;">
            <button class="top-tab-btn active" data-tab="weekly"><?php _e('Semanal', 'lectorthema'); ?></button>
            <button class="top-tab-btn" data-tab="daily"><?php _e('Diario', 'lectorthema'); ?></button>
            <button class="top-tab-btn" data-tab="monthly"><?php _e('Mensual', 'lectorthema'); ?></button>
            <button class="top-tab-btn" data-tab="alltime"><?php _e('Histórico', 'lectorthema'); ?></button>
        </div>
    </div>
