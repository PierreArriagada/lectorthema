<?php
/**
 * MangaNexus - Single Chapter Template (Lector de Capítulos)
 *
 * @package MangaNexus
 */
    ...
    <div class="nexus-container" style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
        <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <span style="font-weight: 700; color: var(--text-primary); font-size: 14.5px;">
                <?php printf(__('Capítulo %s', 'manga-nexus'), esc_html($chapter_num)); ?>
            </span>
        </div>
        ...
        <div style="display: flex; align-items: center; gap: 8px;">
            <?php if ($prev_chap): ?>
                <a href="<?php echo esc_url(get_permalink($prev_chap->ID)); ?>" class="btn-header-action">
                    <i class="fa-solid fa-chevron-left"></i> <?php _e('Anterior', 'manga-nexus'); ?>
                </a>
            <?php endif; ?>
                ...
                <select class="reader-chapter-select" onchange="if (this.value) window.location.href=this.value;">
                    ...
                    ?>
                        <option value="<?php echo esc_url(get_permalink($s_ch->ID)); ?>" <?php selected($s_ch->ID, $chapter_id); ?>>
                            <?php printf(__('Capítulo %s', 'manga-nexus'), esc_html($s_num)); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            <?php endif; ?>
            ...
            <?php if ($next_chap): ?>
                <a href="<?php echo esc_url(get_permalink($next_chap->ID)); ?>" class="btn-header-action btn-auth-primary">
                    <?php _e('Siguiente', 'manga-nexus'); ?> <i class="fa-solid fa-chevron-right"></i>
                </a>
            <?php endif; ?>
        </div>
    </div>
...
<div class="reader-webtoon-container">
    <?php if (!empty($images)): ?>
        <?php foreach ($images as $index => $img_url): ?>
            <img src="<?php echo esc_url($img_url); ?>" alt="<?php printf(__('Página %d', 'manga-nexus'), $index + 1); ?>" class="webtoon-image-page" loading="lazy">
        <?php endforeach; ?>
    <?php else: ?>
        <div style="padding: 60px 20px; text-align: center; color: var(--text-secondary); background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius-sm); margin: 30px 0; width: 100%;">
            ...
            <h3 style="color: var(--text-primary); margin-bottom: 6px;"><?php _e('Páginas en proceso de carga', 'manga-nexus'); ?></h3>
            <p style="color: var(--text-muted); font-size: 13.5px;"><?php _e('Este capítulo aún no tiene páginas añadidas por el editor.', 'manga-nexus'); ?></p>
        </div>
    <?php endif; ?>
</div>
...
<div class="reader-bottom-nav">
    <?php if ($prev_chap): ?>
        <a href="<?php echo esc_url(get_permalink($prev_chap->ID)); ?>" class="btn-hero-info">
            <i class="fa-solid fa-chevron-left"></i> <?php _e('Capítulo Anterior', 'manga-nexus'); ?>
        </a>
    <?php endif; ?>

    <?php if ($parent_manga): ?>
        <a href="<?php echo esc_url(get_permalink($parent_manga->ID)); ?>" class="btn-hero-read">
            <i class="fa-solid fa-list"></i> <?php _e('Índice de Capítulos', 'manga-nexus'); ?>
        </a>
    <?php endif; ?>

    <?php if ($next_chap): ?>
        <a href="<?php echo esc_url(get_permalink($next_chap->ID)); ?>" class="btn-hero-info">
            <?php _e('Siguiente Capítulo', 'manga-nexus'); ?> <i class="fa-solid fa-chevron-right"></i>
        </a>
    <?php endif; ?>
</div>
