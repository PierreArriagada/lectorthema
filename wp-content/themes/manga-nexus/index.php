<?php
/**
 * MangaNexus - Main Index Template
 *
 * @package MangaNexus
 */
    ...
    <div class="nexus-container">
        <div class="section-header-block">
            <h1 class="section-title">
                <?php
                if (is_search()) {
                    printf(__('Resultados para: "%s"', 'manga-nexus'), get_search_query());
                } elseif (is_archive()) {
                    the_archive_title();
                } else {
                    _e('Catálogo de Obras', 'manga-nexus');
                }
                ?>
            </h1>
        </div>
        ...
        <?php else: ?>
            <div style="text-align: center; padding: 60px 20px; background: var(--bg-surface); border-radius: var(--radius-lg);">
                ...
                <h2 style="font-size: 20px; color: #fff; margin-bottom: 8px;"><?php _e('No se encontraron resultados', 'manga-nexus'); ?></h2>
                <p style="color: var(--text-secondary);"><?php _e('Intenta buscar con otros términos o explorar los géneros en la página principal.', 'manga-nexus'); ?></p>
            </div>
        <?php endif; ?>
    </div>
</main>
