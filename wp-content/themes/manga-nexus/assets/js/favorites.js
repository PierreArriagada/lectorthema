/**
 * MangaNexus - Sistema de Favoritos y Marcadores AJAX
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    btn.addEventListener('click', async (e) => {
      ...
      if (!mangaNexusData.isLoggedIn) {
        ...
        if (authModal) {
          ...
          alert(mangaNexusData.strings.loginRequired);
        }
        return;
      }
      ...
      try {
        ...
        formData.append('action', 'manga_nexus_toggle_favorite');
        formData.append('security', mangaNexusData.nonce);
        formData.append('manga_id', mangaId);

        const response = await fetch(mangaNexusData.ajaxUrl, {
          method: 'POST',
          body: formData
        });
        ...
        if (data.success) {
          ...
          showMangaToast(data.data.message || mangaNexusData.strings.errorOccurred, 'error');
          btn.innerHTML = originalHtml;
        }
      } catch (err) {
        console.error(err);
        showMangaToast(mangaNexusData.strings.errorOccurred, 'error');
        btn.innerHTML = originalHtml;
      } finally {
        btn.disabled = false;
        btn.style.opacity = '1';
  ...
  function showMangaToast(message, type = 'success') {
    let toast = document.getElementById('mangaNexusToast');
    if (!toast) {
      toast = document.createElement('div');
      toast.id = 'mangaNexusToast';
      toast.style.cssText = `
        position: fixed;
        bottom: 25px;
        right: 25px;
