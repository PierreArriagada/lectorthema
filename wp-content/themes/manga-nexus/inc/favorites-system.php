<?php
/**
 * MangaNexus - Sistema de Favoritos y Marcadores con Alerta de Nuevos Capítulos
 *
 ...
 */
function manga_nexus_init_favorites_table() {
    global $wpdb;
...
add_action('after_switch_theme', 'manga_nexus_init_favorites_table');
add_action('admin_init', 'manga_nexus_init_favorites_table');

/**
 * 2. Comprobar si un manga es favorito del usuario actual
 */
function manga_nexus_is_favorite($manga_id, $user_id = 0) {
    if (!$user_id) {
 ...
 */
function manga_nexus_get_favorites_count($manga_id) {
    global $wpdb;
    $table = $wpdb->prefix . 'manga_favorites';
 ...
 */
function manga_nexus_ajax_toggle_favorite() {
    check_ajax_referer('manga_nexus_nonce', 'security');

    if (!is_user_logged_in()) {
        wp_send_json_error([
            'message' => __('Debes iniciar sesión para agregar a tus favoritos.', 'manga-nexus'),
            'require_login' => true
        ]);
    }
    ...
    if (!$manga_id) {
        wp_send_json_error(['message' => __('ID de manga no válido.', 'manga-nexus')]);
    }
    ...
    if ($existing) {
        ...
        $is_fav = false;
        $msg = __('Eliminado de tus favoritos.', 'manga-nexus');
    } else {
        // Agregar a favoritos
        $wpdb->insert($table, [
            'user_id'            => $user_id,
        ...
        $is_fav = true;
        $msg = __('¡Agregado a tus favoritos! Te avisaremos cuando haya nuevos capítulos.', 'manga-nexus');
    }

    $total_favs = manga_nexus_get_favorites_count($manga_id);
...
add_action('wp_ajax_manga_nexus_toggle_favorite', 'manga_nexus_ajax_toggle_favorite');

/**
 * 5. Obtener los mangas favoritos de un usuario con indicador de alerta
 */
function manga_nexus_get_user_favorites_list($user_id = 0) {
    if (!$user_id) {
        $user_id = get_current_user_id();
    }
    if (!$user_id) {
 ...
 */
function manga_nexus_ajax_mark_chapter_read() {
    check_ajax_referer('manga_nexus_nonce', 'security');
...
add_action('wp_ajax_manga_nexus_mark_read', 'manga_nexus_ajax_mark_chapter_read');
