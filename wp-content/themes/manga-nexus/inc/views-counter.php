<?php
/**
 * MangaNexus - Sistema de Conteo de Vistas y Tops Dinámicos
 *
 ...
 */
function manga_nexus_init_views_table() {
    global $wpdb;
...
add_action('after_switch_theme', 'manga_nexus_init_views_table');
add_action('admin_init', 'manga_nexus_init_views_table');

/**
 * 2. Registrar Vista de Manga
 */
function manga_nexus_record_view($manga_id) {
 ...
 */
function manga_nexus_get_total_views($manga_id) {
    global $wpdb;
    $table = $wpdb->prefix . 'manga_views';
 ...
 */
function manga_nexus_format_views($number) {
    if ($number >= 1000000) {
 ...
 */
function manga_nexus_fallback_top_mangas($existing_ids, $limit) {
 ...
 */
function manga_nexus_get_top_daily($limit = 9) {
    global $wpdb;
    $table = $wpdb->prefix . 'manga_views';
    ...
    if (count($results) < $limit) {
        $fallbacks = manga_nexus_fallback_top_mangas($existing_ids, $limit);
 ...
 */
function manga_nexus_get_top_weekly($limit = 9) {
    global $wpdb;
    $table = $wpdb->prefix . 'manga_views';
    $posts_table = $wpdb->posts;
 ...
 */
function manga_nexus_get_top_monthly($limit = 9) {
    global $wpdb;
    $table = $wpdb->prefix . 'manga_views';
    $posts_table = $wpdb->posts;
 ...
 */
function manga_nexus_get_top_alltime($limit = 9) {
    global $wpdb;
    $table = $wpdb->prefix . 'manga_views';
    $posts_table = $wpdb->posts;
 ...
 */
function manga_nexus_get_top_by_genre($genre_slug, $limit = 9) {
    $args = [
        'post_type'      => 'manga',
        'post_status'    => 'publish',
