<?php
/**
 * MangaNexus - Template Part: Manga Card (Últimos Capítulos Agregados)
 *
...
<article class="manga-card" data-manga-id="<?php echo esc_attr($manga_id); ?>">
    <div class="manga-card-cover-wrap">
        ...
        <div class="manga-card-top-bar">
            <div class="manga-top-left-group">
                <!-- Puntuación -->
                <span class="manga-rating-pill" title="<?php printf(__('Puntuación: %s / 10', 'manga-nexus'), esc_attr($rating)); ?>">
            ...
            <button type="button" 
                    ...
                    title="<?php echo $is_fav ? esc_attr__('Guardado en Favoritos', 'manga-nexus') : esc_attr__('Guardar en Favoritos', 'manga-nexus'); ?>"
                    aria-label="<?php esc_attr_e('Guardar en favoritos', 'manga-nexus'); ?>">
                <?php echo $is_fav ? manga_nexus_svg('bookmark', 'svg-card-fav', 13) : manga_nexus_svg('bookmark-outline', 'svg-card-fav', 13); ?>
            </button>
        </div>
    ...
    <div class="manga-card-chapters-list">
        <?php if (!empty($latest_chapters)): ?>
            ...
            $ch1_time = manga_nexus_time_ago(get_the_time('U', $ch1->ID));
            ?>
            <a href="<?php echo esc_url(get_permalink($ch1->ID)); ?>" class="chapter-horizontal-row ch-latest-red" title="<?php printf(__('Cap. %s', 'manga-nexus'), esc_attr($ch1_num)); ?>">
                <span class="chap-num">
                    <?php printf(__('Cap. %s', 'manga-nexus'), esc_html($ch1_num)); ?>
                </span>
            ...
            <?php if (isset($latest_chapters[1])): ?>
                ...
                $ch2_time = manga_nexus_time_ago(get_the_time('U', $ch2->ID));
                ?>
                <a href="<?php echo esc_url(get_permalink($ch2->ID)); ?>" class="chapter-horizontal-row ch-prev-neutral" title="<?php printf(__('Cap. %s', 'manga-nexus'), esc_attr($ch2_num)); ?>">
                    <span class="chap-num">
                        <?php printf(__('Cap. %s', 'manga-nexus'), esc_html($ch2_num)); ?>
                    </span>
        ...
        <?php else: ?>
            <div class="chapter-horizontal-row ch-prev-neutral" style="opacity: 0.5; justify-content: center; cursor: default;">
                <span class="chap-num"><?php _e('Próximamente', 'manga-nexus'); ?></span>
            </div>
        <?php endif; ?>
    </div>
</article>
